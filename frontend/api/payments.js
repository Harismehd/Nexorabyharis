...
function calculateMemberStatus(member) {
  ...
  return end > new Date() ? 'Active' : 'Dues';
}
...
function createPaymentRecord(db, { gymKey, memberId, amount, method, monthsCovered }) {
  ...
  // GET /api/payments/pending?gymKey=xxx
  if (req.method === 'GET' && url.includes('pending')) {
    const { gymKey } = req.query;
    ...
    const pending = (db.pendingPayments || [])
      ...
      .map(p => ({ ...p, transactionHash: undefined }));
  ...
  if (req.method === 'POST' && url.includes('proof')) {
    ...
    const duplicateCount =
      db.pendingPayments.filter(p => p.transactionHash === transactionHash).length +
      (db.payments || []).filter(p => p.transactionHash === transactionHash).length;
    if (duplicateCount > 0) return res.status(409).json({ error: 'Transaction ID already exists' });

    const pending = {
      id: uuidv4(), gymKey, memberId,
      amount: String(amount), method, transactionHash,
      transactionLast4: String(transactionId).slice(-4),
      proofNote: proofNote ? String(proofNote).slice(0, 500) : '',
      status: 'pending', verificationStrength: 'high',
  ...
  if (req.method === 'POST' && url.includes('verify')) {
    ...
    const result = createPaymentRecord(db, {
      gymKey, memberId: pending.memberId,
    ...
    return res.json({
      ...
      confirmation: { verificationCode, receiptNumber: result.payment.receiptNumber, paymentDate: result.payment.paymentDate },
