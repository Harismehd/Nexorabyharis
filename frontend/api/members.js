...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    const members = db.members.filter(m => m.gymKey === gymKey).map(m => ({ ...m, status: calculateMemberStatus(m), dueDate: getMemberDueDate(m) }));
    return res.json({ members });
  }

  if (req.method === 'POST') {
    const { gymKey, name, phone, email, joiningDate, subscriptionType, amount } = req.body;
    const newMember = { id: uuidv4(), gymKey, name, phone, email: email || '', joiningDate: joiningDate || new Date().toISOString(), subscriptionType: subscriptionType || 'monthly', subscriptionEndDate: '', amount: amount || 0 };
